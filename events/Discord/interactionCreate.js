...
module.exports = async (client, inter) => {
    await inter.deferReply({ flags: 1 << 6 });
    if (inter.type === InteractionType.ApplicationCommand) {
        ...
        if (!command) {
            errorEmbed.setDescription(await Translate('<❌> | Error! Please contact get in contact with Daniel!'));
            inter.editReply({ embeds: [errorEmbed], flags: 1 << 6 });
        ...
        if (command.permissions && !inter.member.permissions.has(command.permissions)) {
            ...
            return inter.editReply({ embeds: [errorEmbed], flags: 1 << 6 });
        }
